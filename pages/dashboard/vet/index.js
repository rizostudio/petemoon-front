...
import { getListVisit } from "@/services/dashboard/vet/getVisitList";
...
const Vet = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await getListVisit();
        ...
        <div className="flex flex-col my-2 lg:my-4 mb-[100px]">
          {visitData.length ? (
            visitData.map((item, index) => (
              <div
                key={item.id}
                className="flex flex-col lg:flex-row lg:items-center justify-between w-full p-5 lg:px-[65px] lg:py-[10] my-1 lg:my-2 bg-white border-[1px] border-secondary lg:border-none rounded-[15px] lg:rounded-[25px] shadow-shadowB"
              >
                <div className="flex justify-between items-center">
                  ...
                  >
                    <bdi>جزئیات ویزیت</bdi>
                  </Link> */}
                </div>
                {/* mobile */}
                <div className="lg:hidden flex text-[10px] xs:text-sm sm:text-base mt-4">
